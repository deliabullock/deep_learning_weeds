...
IMAGE_SIZE = 299
NUM_TRAIN_IM_NEEDED = 22063
NUM_TEST_IM_NEEDED = 4591 
NUM_VAL_IM_NEEDED = 4595
	    ...
	    response = requests.get(url)
			...
			jitter_directions = []
				...
				if jitter_down and image_grid[r + 1][n + 1]['class'] == curr_class:
				    jitter_diag = True
		        #jitter down
		        dims, warped = get_image_dims(curr_x, curr_y + jitter_step, w, h)
		        if jitter_down and not warped:
			    jitter_directions.append('down')
			    if pics_created < num_im_needed and curr_class == 1 and not warped:
				curr_y_tmp = curr_y
		    	        for x in range(1):
				    pics_created += 1
		        ...
		        #jitter right
		        dims, warped = get_image_dims(curr_x + jitter_step, curr_y, w, h)
		        if jitter_right and not warped:
			    ...
			    if pics_created < num_im_needed and curr_class == 1:
				    ...
				    pics_created += 1
		        	    curr_x_tmp = curr_x_tmp + jitter_step
                    		    ...
                    		    croppedim.save(imageName)
		        #jitter diagonal
		        dims, warped = get_image_dims(curr_x + jitter_step, curr_y + jitter_step, w, h)
		        if jitter_diag and not warped:
    ...
    def crop_image(self, url, data_dir, imagenum, rand_x, rand_y):
	    ...
	    picture_info = {
            ...
            for r in range(num_rows):
                curr_x = x_start
                for n in range(num_colu):
		    dims, warped = get_image_dims(curr_x, curr_y, w, h)
		    if not warped:
		    	croppedim = im.crop(dims)
                    	croppedim = croppedim.resize((300, 300), Image.ANTIALIAS)
