...
class csvreader(object):
	...
	i = 0
        ...
        f = open(weed_pkl, 'ab')
        pickle.dump(weed_nums, f)
        f.close()
        f = open(nonweed_pkl, 'ab')
        pickle.dump(nonweed_nums, f)
        f.close()
        f = open(x_y_pkl, 'ab')
        pickle.dump(out_x_y, f)
        f.close()
    ...
    def crop_image(self, url, imagenum):
	    ...
	    out_x_y.append([url, rand_x, rand_y)
	    ...
	    weed_pictures = []
			...
			img = im.crop((curr_x, curr_y, curr_x + 298, curr_y + 298))
			weed_pictures.append([img, curr_x, curr_y])
	    ...
	    weed_classes = model.predict_classes(weed_images, batch_size=10)
            for i in len(weed_pictures):
		croppedim = weed_pictures[i][0]
		x = weed_pictures[i][1]
		y = weed_pictures[i][2]
		if weed_classes[i] == 1:
                        croppedim.save(weed_dir + 'img'+str(imagenum)+'.jpg')
			weed_nums.add('img'+str(imagenum)+'.jpg')
			draw.polygon([(x, y),(x + 298, y),(x + 298, y + 298),( x, y + 298)], (150,0,255,50))
		else:
                        croppedim.save(nonweed_dir + 'img'+str(imagenum)+'.jpg')
			nonweed_nums.add('img'+str(imagenum)+'.jpg')
